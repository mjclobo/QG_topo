...
while ss_yr_cnt < ss_yr_max
    ...
    if j % nsubs == 0
        ...
        if ((t_yrly[end] - yr_cnt*365*86400) > 0)
            ...
            if KE[1,end]/KE[1,1] < KE_thresh && ss_yr==false
                global ss_yr = true
                global ss_yr_cnt = 1
            end
            ...
            if topo_type=="y_slope"
                jld_name = data_dir*"/threelayer_h0"* string(round(h0*Lx,digits=9))*"_kt"* string(Int(kt)) *"_res" * string(Int(Nx)) * string(Int(Nx)) *"_yr"*string(yr_cnt)*  ".csv"
            elseif topo_type=="sin_sin"
                jld_name = data_dir*"/threelayer_h0"* string(Int(h0[1]))*"_kt"* string(Int(kt[1])) *"_res" * string(Int(Nx)) *"_yr"*string(yr_cnt)*  ".csv"  
            else
                jld_name = data_dir*"/threelayer_h0"* string(Int(h0))*"_kt"* string(Int(kt)) *"_res" * string(Int(Nx)) *"_yr"*string(yr_cnt)*  ".csv"
            end
